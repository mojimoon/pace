...
def kfold_cross(data):
    ...
    unique_times = list(range(1,76))  # 75 is # of timesections in Dataset1 and Dataset 2 combined
...
def read_images(image_folder, camera, ids, image_size):
    ...
    for id in ids:
        ...
        # Resizing
        img = resize(crop_img, size=image_size)
    ...
    while True:
        ...
        if steerings[i] and image_stamps[i]:
            ...
            if buffer_size >= batch_size:
                ...
                yield x.astype(np.float32), preprocess_output(y)
